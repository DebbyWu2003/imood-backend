...
- [x] 首輪數據（FLEURS 15 句真人朗讀）：VAD 句尾判定 ~700ms、ASR ~2000ms
      (RTF 0.40x)、LLM 首字 ~145ms / 全程 ~2200ms、**講完→回覆完 中位 5.4s**
      （~5s 切段；casual 短句估 ~3–3.5s）。
- [x] 瓶頸：ASR 與 LLM 全程各佔一半，VAD 最小。槓桿見文件第 7 節。
- [x] 斷句行為：700ms 對朗讀長句偏短（15 句切了 8 句）→ 傾向提到 900–1000ms。
- [ ] **待做**：真人**對話**語音（自然停頓 / 口語 / 語助詞 / 噪音）測斷句準度，
      FLEURS 朗讀代替不了。依此現場調 VAD 參數並定案。
