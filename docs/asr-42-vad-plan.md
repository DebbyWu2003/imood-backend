...
| **句尾靜音（最關鍵）** | **900ms** 連續靜音 | 中文句間停頓 ~300–500ms、思考停頓可到 ~1s。700ms 對朗讀長句太容易切段（見第 7 節），2026-08-29 調到 900 |
...
- **測試**：
  ...
  - `scripts/measure_e2e.py` —— 4.5 延遲量測：把語料以真實時間速率串進
    `/ws/audio`，用回傳訊息時間戳拆解 VAD / ASR / LLM 各段延遲。
  - 瀏覽器實測：真的連 `/ws/audio` 串 TTS 語音 → DOM 出現 user 訊息 + 逐字
    回覆 + 表情切換。
- **尚未驗證**：真人**對話**語音下的斷句準度與 VAD 參數（4.5 目前只跑過
  FLEURS 朗讀，見第 7 節）。barge-in、雙講不處理。

---

## 7. 4.5 延遲量測 —— 首輪（FLEURS 真人朗讀，2026-08-29）

`scripts\measure_e2e.py samples\fleurs\manifest.jsonl`（15 句，真實時間速率）。
FLEURS 是**朗讀新聞/百科**，不是自然對話，數據當粗略基準看。
faster-whisper `small` / CPU / int8，Qwen2.5-1.5B / CPU。

### `end_silence_ms` 700 vs 900 對照

| | 700ms | **900ms（採用）** |
|---|---|---|
| 15 句裡被切段的 | 8 句（23 utterance） | **5 句（20 utterance）** |
| VAD 句尾判定 中位 | 661ms | 991ms |
| ASR 辨識 中位 | 1975ms | 2052ms |
| LLM 首字 中位 | 137ms | 156ms |
| LLM 全程 中位 | 2292ms | 1876ms（變異大，一次 7.8s 離群） |
| **講完→回覆完 中位** | 5431ms | **5021ms** |

- 900ms 少切 3 句，端到端延遲實際上沒變差（LLM 變異蓋過 +300ms 的 VAD 差）。
- `EndpointConfig.end_silence_ms` 預設已改成 900。
- **900ms 仍沒完全消除切段**——FLEURS 長句朗讀還有 5 句有 >900ms 停頓。
  更長（1200ms+）會讓短回覆感覺遲鈍；真正的解法是「transcript 後短暫等一下
  看有沒有續句，有就併起來再丟 LLM」，那個之後有需要再做。

### 各段延遲（900ms，ms）

| 段 | 平均 | 中位 | 範圍 |
|---|---|---|---|
| VAD 句尾判定 | 947 | 991 | 660–996 |
| ASR 辨識 | 2049 | 2052 | 1696–2388 |
| LLM 首字 | 172 | 156 | 83–424 |
| LLM 全程 | 2307 | 1876 | 321–7831 |
| **講完 → 回覆完** | 5472 | **5021** | 3149–11105 |

- 「講完 → 看到回覆開始」≈ 900 + 2050 + 170 ≈ **3.1s**。
- 這是 ~5–6s 切段的數字；真實 casual 短句（2–3s）ASR 等比例縮短 → 端到端估
  **~3–3.5s**。ASR RTF ≈ **0.34–0.40x**。

### 瓶頸

**ASR (~2s) 和 LLM 全程 (~2s) 差不多大，各佔一半；VAD (~1s) 最小。**
要壓延遲的槓桿：
- ASR：`beam_size` 5 → 1（greedy，小幅品質換速度）、或上 GPU。
- LLM：`max_tokens` 200 → 128 + 收緊 prompt（同時解決回覆偏長偏說教）。
